...
void handleWasmFileUpload() {
    ...
    String fname(TASK_BASE_FOLDER + httpUpload.filename);
...
void handleTaskStart() {
    ...
    if(!doesTaskDetailsExist(filename)) {
        Serial.println("task details not found");
        Serial.println(filename.c_str());
        server.send(
            404, 
            "application/json", 
            createErrorMessage("task details not found")
        );
        return;
    }
